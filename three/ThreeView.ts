...
export let DeviceOrientationControls:IDeviceOrientationControls = (THREE as any).DeviceOrientationControls;
...
export class ThreeView
{
	...
	get stereoEffect ():IStereoEffect { return this._stereoEffect; }
	 ...
	 */
	get viewPortSize ():IViewportSize { return this._viewPortSize; }
	...
	get onEnterFramed ():Signal { return this._onEnterFramed; }

	/**
	 * Scene clock. Started when rendering is not paused.
	 */
	protected _clock				:THREE.Clock;
	get clock ():THREE.Clock { return this._clock; }
	...
	)
	{
		...
		if (this._renderer != null) throw new Error(`ThreeView.initRenderer // Already initialised.`);
		...
		this._renderer = (
			hasWebGL
				...
				: new THREE.CanvasRenderer( pRendererParams )
		) as THREE.Renderer;
		...
		if (pAutoSize)
		{
			ResponsiveManager.instance.onWindowSizeChanged.add(this.windowSizeChangedHandler, this);
		}
	...
	{
		// Check if init order is ok
		if (this._renderer == null) throw new Error(`ThreeView.initControls // Please init renderer before controls`);
		if (this._scene != null) throw new Error(`ThreeView.initControls // Please init controls before scene`);
	...
	initScene (
		...
		pFog			:IFogConfig 		= {near: 1, far: 4000, color: 0xFFFFFF}
	)
	{
		// Check if init order is ok
		if (this._renderer == null) throw new Error(`ThreeView.initScene // Please init renderer before scene`);
		if (this._scene != null) throw new Error(`ThreeView.initScene // Scene already init.`);
	...
	start (pParentDomElement:HTMLElement)
	{
		...
		if (this._renderer == null) throw new Error(`ThreeView.initScene // Please init renderer before starting 3D view.`);
